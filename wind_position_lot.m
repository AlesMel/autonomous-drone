...
%% 
% Example Data
strength = [0.207034, 0.1983741, 0.3085291, 0.1402218, 0.1605727, 0.007303238];
quaternions = [
...
% Quaternion to direction vectors conversion
directions = zeros(size(quaternions, 1), 3);
for i = 1:size(quaternions, 1)
    ...
    % Apply rotation to a standard axis vector (z-axis here)
    vec = R * [0; 0; 1];
    % Scale each vector by its corresponding strength
    directions(i, :) = vec' * strength(i);
end

% Plot using quiver3
quiver3(zeros(size(directions, 1), 1), zeros(size(directions, 1), 1), zeros(size(directions, 1), 1), ...
        directions(:, 1), directions(:, 2), directions(:, 3), 0, 'AutoScale', 'off', 'Color', 'k', 'LineWidth', 2);  % Use '0' for fixed length
hold on
%% 
data_ppo = readmatrix('wind_ppo.csv');
cutoff_index = 1001;
data_ppo = data_ppo(3:cutoff_index, :);
total_distance_ppo = 0;
for i = 2:size(data_ppo, 1)
    total_distance_ppo = total_distance_ppo + norm(data_ppo(i, :) - data_ppo(i-1, :));
end
...
start = [0, 0, 0];
plotCube(start, 0.1, 'y', 1); % Green cube of size 1.5
...
hStart =fill3(NaN, NaN, NaN, 'y', 'EdgeColor', 'none');

hold off

legend([t1, t2, hStart], sprintf('Trajektória PPO, vzdialenosť: %.2f [m]', total_distance_ppo), sprintf('Trajektória NEAT, vzdialenosť: %.2f [m]', total_distance_neat), 'Cieľ')
