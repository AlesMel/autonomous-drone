...
namespace DroneProject
{
    ...
    public class Drone : MonoBehaviour
    {
        ...
        public void Initialize()
        {
            ...
            m_Rotors = GetComponentsInChildren<Rotor2>();
        ...
        public void ApplyActions()
        {
            ...
            for (int i = 0; i < 4; i++)
            {
                ...
                Debug.Log("Action: " + actions.ToString());
                // Thrust per rotor.
                ...
                float actionWithDirection = actions[i] * m_RotorTurnDirections[i];

                // torqueAxis = m_Rotors[i].LocalTorqueAxis;
                m_Rigidbody.AddRelativeTorque(actionWithDirection * m_TorqueFactor * torqueAxis);
                
                // Buffer value for animation.
                m_AnimationSpeeds[i] = actionWithDirection;
            }
        ...
        private void FixedUpdate()
        {
            ...
            if (m_Animate)
            {
                float maxSpeed = Time.fixedDeltaTime * m_AnimSpeedFactor;
                for (int i = 0; i < 4; i++)
                {
                    m_Rotors[i].Blade.Rotate(0, m_AnimationSpeeds[i] * maxSpeed, 0, Space.Self);
                }
            }
        }
